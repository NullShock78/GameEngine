...
{
	GameLogger::Log(LogMsgType::cInfo, "Shader Init started");
	...
	GameLogger::Log(LogMsgType::cInfo, "Shader Program created.");
	...
	m_id = glCreateResult;
	m_in_use = true;
	GameLogger::Log(LogMsgType::cProcess, "ShaderProgram::Initialize()[%u] successful!",m_id);
	return true;
}
...
bool ShaderProgram::Shutdown()
{
	...
	GameLogger::Log(LogMsgType::cProcess, "ShaderProgram[%u]::Shutdown successful!", m_id);
	return true;
}
...
bool ShaderProgram::AddShader(const char * const filename, GLenum shaderType)
{
	...
	GameLogger::Log(LogMsgType::cDebug, "ShaderProgram::AddShader() : glCompileShader() successful");
	delete[] fileContents;

	// CheckShaderCompileStatus
	if (!CheckShaderCompileStatus(shaderID))
	{
		GameLogger::Log(LogMsgType::cError, "ShaderProgram::AddShader [%u] CheckShaderCompileStatus() bad.", shaderID);
		return false;
	}
	GameLogger::Log(LogMsgType::cDebug, "ShaderProgram::AddShader() : glCompileShader() successful");
	...
	GameLogger::Log(LogMsgType::cDebug, "ShaderProgram::AddShader() : glAttachShader() successful");
...
bool ShaderProgram::CheckProgramStatus(GLenum pname)
{
	...
	GameLogger::Log(LogMsgType::cInfo, "CheckProgramStatus() successful!" );//remember pname
	return true;
}
...
bool ShaderProgram::LinkAndUseProgram()
{
	...
	GameLogger::Log(LogMsgType::cInfo, "Shader Program [%u] linked and used successfully.", m_id);
	return true;
}
...
bool ShaderProgram::SaveShaderInfo(const char * const filename, GLenum shaderType, GLuint shaderId)
{
	...
	GameLogger::Log(LogMsgType::cInfo, "ShaderInfo saved successfully.");
	return true;
}
...
bool ShaderProgram::DeleteShaders()
{
	...
	for (GLuint j = 0; j < maxShaders; ++j)
	{
		...
		if (m_shaders[j].in_use) {
			GameLogger::Log(LogMsgType::cInfo, "Shader  [%d] deleting...", m_shaders[j].id);
			...
			{
				GameLogger::Log(LogMsgType::cInfo, "Shader  [%d] failed to delete.", m_shaders[j].id);
				return false;
			}
			//GameLogger::Log(LogMsgType::cInfo, "Shader [%d] deleted successfully", j);
			m_shaders[j].in_use = false;
			GameLogger::Log(LogMsgType::cInfo, "Shader  [%d] deleted successfully.", m_shaders[j].id);
		}
	}
...
bool ShaderProgram::DeleteProgram()
{
	...
	if (m_in_use)
	{
		...
		{
			GameLogger::Log(LogMsgType::cInfo, "Program [%u] failed to delete.", m_id);
			return false;
		}
