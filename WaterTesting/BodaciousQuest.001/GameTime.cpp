...
bool GameTime::Initialize ()
    ...
    if (f_isInitialized)
        {
        GameLogger::Log(LogMsgType::cProgrammerError, "GameTime was already initialized!");
        return false;
        }
    ...
    if ( f_timerCountsPerSecond != 0 )
        ...
        {
        GameLogger::Log(LogMsgType::cFatalError, "GameTime::Initialize(). Hardware doesn't support high-resolution performance counters");
        return false;
        }

    GameLogger::Log(LogMsgType::Process, "GameTime::Initialize() ok\n");
    return true;
    }

bool GameTime::Shutdown ()
    {
    GameLogger::Log(LogMsgType::Process, "GameTime::Shutdown() ok\n");
    return true;
    }
...
bool LogWindowsError ()
    ...
    if ( result != 0 )
        {
        GameLogger::Log(LogMsgType::cError, reinterpret_cast<LPCSTR>( messageBuffer ));				
        if ( LocalFree( messageBuffer ) != NULL )
            {
            GameLogger::Log(LogMsgType::cError, "GameTime: Windows couldn't deallocate error message memory");
            return false;
            }
        return true;
        }

    GameLogger::Log(LogMsgType::cError, "GameTime: Couldn't format Windows error message");
    return true;
    }
