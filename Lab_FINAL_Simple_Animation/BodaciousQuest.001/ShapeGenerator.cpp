...
Mesh ShapeGenerator::m_normalPlaneMesh;
...
VertNorm ShapeGenerator::generated_plane_normVerts[6];
...
GLushort ShapeGenerator::m_debug_cube_indices[24] =
{
 ...
 float ShapeGenerator::normal_plane_normVerts[36] = {
 ...
 bool ShapeGenerator::Initialize(ConfigReader * config)
 {
	 ...
	 m_normalPlaneMesh.SetData(&normal_plane_normVerts, 6, nullptr, 0, IndexSizeInBytesEnum::Uint, glm::vec3(0,0,0), VertexFormats::PositionNormal);
	 m_normalPlaneMesh.GetRenderInfo()->m_indexed = false; //TODO: move
	 m_normalPlaneMesh.GetRenderInfo()->m_shader = 4;
	 m_normalPlaneMesh.GetRenderInfo()->m_wroteShader = 4;

	 if (!RenderEngine::AddMesh(&m_normalPlaneMesh, 4, GL_TRIANGLES, VertexFormats::PositionNormal))return false;
	 //-------------------------------------------------------------------------------------------------------------------------
	 //-------------------------------------------------------------------------------------------------------------------------
	 //-------------------------------------------------------------------------------------------------------------------------
	 //-------------------------------------------------------------------------------------------------------------------------
 ...
 void ShapeGenerator::MakeNormaledPlane(GraphicalObject * gob)
 {
	 gob->SetMesh(&m_normalPlaneMesh);
 ...
 bool ShapeGenerator::CreatePlane(GraphicalObject * gob, glm::vec2 upperLeft, glm::vec2 lowerRight, glm::vec3 tint)
 {

	 glm::vec3 up = glm::vec3(0, 1.0f, 0);

	 glm::vec3 upperL = glm::vec3(upperLeft.x, 0.0f, upperLeft.y);
	 glm::vec3 lowerR = glm::vec3(lowerRight.x, 0.0f, lowerRight.y);
	 glm::vec3 upperR = glm::vec3(lowerR.x, 0.0f, upperL.z);
	 glm::vec3 lowerL = glm::vec3(upperL.x, 0.0f, lowerR.z);

	 
	 //glm::vec3 vecA = upperR - upperL;


	 generated_plane_normVerts[0] = { upperL, up };
	 generated_plane_normVerts[1] = { upperR, up };
	 generated_plane_normVerts[2] = { lowerL, up };
	
	 
	 generated_plane_normVerts[3] = { lowerR, up };
	 generated_plane_normVerts[4] = { lowerL, up };
	 generated_plane_normVerts[5] = { upperR, up };


	 gob->SetTint(tint);
	 Mesh * mesh = new Mesh;
	 mesh->SetData(generated_plane_normVerts, 6, nullptr, 0, IndexSizeInBytesEnum::Ubyte, glm::vec3(), VertexFormats::PositionNormal);
	 mesh->m_renderInfo.m_indexed = false;
	 if (!RenderEngine::AddMesh(mesh, 4, GL_TRIANGLES, VertexFormats::PositionNormal)) return false;
	 gob->SetMesh(mesh);
	 gob->SetHasData(true);
