...
bool BufferInfo::Initialize()
{
	...
	GameLogger::Log(LogMsgType::Process, "BufferInfo::Initialize() successful.");
	return true;
}
...
bool BufferInfo::AddMesh(Mesh * mesh, GLenum drawType,VertexFormats vertFormat)
{
	...
	if (m_headRenderInfoPtr == nullptr)
	{
		...
		mesh->m_renderInfo.m_next = nullptr;
		indexOffsetBytes = 0;
		vertOffset = 0;
...
bool BufferInfo::CreateBuffers()
{
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_indexBufferID);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_indexBufferSize, NULL, GL_STATIC_DRAW);
