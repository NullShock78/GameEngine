...
glm::mat4 Camera::GetWorldToViewMatrix(glm::vec3 pos)
{
	return glm::lookAt(pos, pos + m_viewDir, m_up);
}

glm::mat4 Camera::GetReflectionToViewMatrix()
{
...
void Camera::LookAt(glm::vec3 point)
{
	...
	m_viewDir = glm::normalize( point - m_position);
...
glm::mat4 Camera::GetViewAt(glm::vec3 pos)
{
	glm::vec3 temp = glm::normalize(pos - m_position);

	return glm::lookAt(m_position, m_position + temp, m_up);
...
void Camera::Reflect2(glm::vec3 otherCameraDir )
{
	...
	m_viewDir = glm::normalize(glm::reflect(glm::normalize(otherCameraDir), glm::vec3(0, 1.0f, 0)));   //glm::vec3(glm::rotate(glm::mat4(), glm::radians(180.0f),m_initialDir) *-glm::vec4(m_position - otherCameraDir, 1.0f)); //glm::reflect(glm::normalize(   m_position - otherCameraDir), m_initialDir);
}
...
glm::mat4 Camera::GetViewWithRot(float rot)
{
	glm::vec3 forw = glm::normalize(glm::vec3(m_viewDir.x, 0.0f, m_viewDir.z));
	//glm::vec3 forw = glm::normalize(glm::vec3(m_viewDir.x, 0.0f, m_viewDir.z));

	glm::vec3 f = glm::vec3(glm::rotate(glm::mat4(),glm::radians(rot), forw) * glm::vec4(m_up, 1.0f));//glm::normalize(glm::vec3(m_viewdir.x, 0.0f, m_viewDir.y));

	return glm::lookAt(m_position, m_position + m_viewDir, f);
}
