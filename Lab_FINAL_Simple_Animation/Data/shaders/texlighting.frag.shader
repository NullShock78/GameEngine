...
void main()
{
	...
	vec3 ambient = tex * ambientRef * ambientCol;
	...
	vec3 diffuse = tex * diffuseCol * diffuseRef * cosTheta * att;

	vec3 E = normalize(eyeDir);
	vec3 R = reflect(-l, n);
	float cosAlpha = clamp(dot(E, R), 0.0, 1.0);

	vec3 specular = tex * specCol  * clamp(specularityRef * (pow(cosAlpha, specularityShininess)) * att, 0.0, 1.0);//vec3(1.0,1.0,1.0) * tex * ((reflectVec.x * vVec.x) + (reflectVec.y * vVec.y) + 
